...
const POSTING_SOURCES = [
  ...
  // 'admission' — registration fees taken at admission time. Deliberately NOT
  // folded into 'fee': the idempotency index is (school, source, sourceId), and
  // the deleted-receipt reconciliation walks every 'fee' claim and compares it
  // against what /fees/students and /fees/assignments return. An admission
  // posting filed under 'fee' appears in neither, so every one of them would be
  // reported as a deleted receipt. A separate source keeps that comparison
  // honest and costs one enum value.
  'admission',
];
