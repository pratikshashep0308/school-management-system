...
exports.getDashboardStats = async (req, res) => {
  ...
  const [
    ...
    // Count every payment received this month. Filtering on status:'paid'
    // excluded 'partial' instalments — money that was genuinely collected —
    // so the figure came out far lower than the real total.
    FeePayment.aggregate([
      { $match: { school, createdAt: { $gte: new Date(new Date().setDate(1)) } } },
  ...
  res.status(200).json({
    ...
    data: {
      ...
      totalTeachers,
      attendanceRate,
      // Raw counts so the attendance panel can show real figures instead of 0
      todayPresent,
      todayTotal,
