...
export default function StudentAttendance() {
  ...
  const loadStudents = async () => {
    ...
    try {
      ...
      // build student list from records + fill any not yet marked
  ...
  useEffect(() => { if (classId) loadStudents(); }, []);
  ...
  const handleSubmit = async () => {
    if (!students.length) return toast.error('No students loaded. Click Submit after selecting class.');
    setSaving(true);
    try {
    ...
    <div style={{ maxWidth:860 }}>
      ...
      <div style={{ background:'#fff', border:'1px solid #E5E7EB', borderRadius:14, padding:28, marginBottom:20 }}>
        ...
        <button onClick={() => { loadStudents(); }} disabled={loading}
          style={{ padding:'10px 32px', borderRadius:20, background:'#F59E0B', color:'#fff', border:'none', fontSize:13, fontWeight:700, cursor:'pointer', display:'flex', alignItems:'center', gap:8 }}>
        ...
        <div style={{ background:'#fff', border:'1px solid #E5E7EB', borderRadius:14, overflow:'hidden' }}>
          ...
          <div style={{ padding:'12px 20px', borderBottom:'1px solid #F3F4F6', display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:10 }}>
            <div style={{ fontWeight:700, fontSize:14, color:'#111827' }}>
              {classes.find(c=>c._id===classId)?.name} — {students.length} students
              <span style={{ fontSize:12, color:'#6B7280', fontWeight:400, marginLeft:8 }}>{date}</span>
            </div>
            <div style={{ display:'flex', gap:6 }}>
              {['present','absent','leave'].map(s=>(
                <button key={s} onClick={()=>setAll(s)}
                  style={{ padding:'5px 14px', borderRadius:20, fontSize:12, fontWeight:700, border:'none', cursor:'pointer', background: STATUS_STYLE[s].bg, color: STATUS_STYLE[s].color, textTransform:'capitalize' }}>
                  All {s}
                </button>
              ))}
            </div>
          ...
          <table style={{ width:'100%', borderCollapse:'collapse', fontSize:13 }}>
            <thead>
              <tr style={{ background:'#0B1F4A' }}>
                {['#','Roll No','Student','Status','Actions'].map(h=>(
              ...
              {students.map((s,i)=>(
                <tr key={s._id} style={{ borderBottom:'1px solid #F3F4F6', background: i%2?'#FAFAFA':'#fff' }}>
                  ...
                  <td style={{ padding:'10px 16px' }}>
                    <span style={{ fontSize:11, fontWeight:700, padding:'3px 12px', borderRadius:20, background: STATUS_STYLE[statuses[s._id]]?.bg||'#F3F4F6', color: STATUS_STYLE[statuses[s._id]]?.color||'#9CA3AF', textTransform:'capitalize' }}>
                      {statuses[s._id] || '—'}
                    </span>
                  </td>
                  <td style={{ padding:'10px 16px' }}>
                    <div style={{ display:'flex', gap:6 }}>
                      {STATUS_OPTS.map(st=>(
                        <button key={st} onClick={()=>setStatuses(p=>({...p,[s._id]:st}))}
                          style={{ padding:'4px 10px', borderRadius:6, fontSize:11, fontWeight:700, border:'2px solid', cursor:'pointer', textTransform:'capitalize',
                            borderColor: statuses[s._id]===st ? STATUS_STYLE[st].bg : '#E5E7EB',
                            background:  statuses[s._id]===st ? STATUS_STYLE[st].bg : '#fff',
                            color:       statuses[s._id]===st ? STATUS_STYLE[st].color : '#374151',
                          }}>
