...
function DateWiseHistory({ studentId, dashboardAttendance }) {
  ...
  const load = async () => {
    setLoading(true);
    try {
      // Use studentPortalAPI — only returns student's own data
      ...
      // 403 = fallback to dashboardAttendance records
      if (err?.response?.status === 403 && dashboardAttendance?.records?.length) {
        const all = dashboardAttendance.records;
        const from = new Date(dateFrom); from.setHours(0,0,0,0);
        const to   = new Date(dateTo);   to.setHours(23,59,59,999);
        const filtered = all.filter(rec => {
          const d = new Date(rec.date);
        ...
        setSummary({ present, absent, late:0, excused:0, total, pct });
      } else {
        setRecords([]);
        setSummary(null);
      }
...
export default function StudentAttendanceSection({ dashboardAttendance, studentId }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      // 403 = student portal not accessible (role mismatch) - silently use dashboard data
      if (err?.response?.status !== 403) {
        toast.error('Failed to load attendance');
      }
      // Fall back to dashboard data passed as prop
      if (dashboardAttendance) {
        setData({ summary: dashboardAttendance, records: dashboardAttendance.records || [] });
      }
    }
    finally { setLoading(false); }
