...
export default function StudentAttendanceReport() {
  ...
  const generate = async () => {
    ...
    try {
      ...
      const month = from.getMonth()+1, year = from.getFullYear();
      const r = await attendanceAPI.getClassAnalytics(classId, month, year);
      ...
      (data.breakdown||[]).forEach(entry => {
        Object.entries(entry.days||{}).forEach(([day, status]) => {
          ...
          if (date >= from && date <= to) {
            flat.push({
              date, dateStr: date.toLocaleDateString('en-IN',{day:'2-digit',month:'short',year:'numeric'}),
              sortDate: date.toISOString(),
              day: date.toLocaleDateString('en-IN',{weekday:'long'}),
              id: entry.student?.admissionNumber || entry.student?.rollNumber || '—',
  ...
  const filtered = rows.filter(r=>{
    ...
    return !q||r.name.toLowerCase().includes(q)||r.id.toLowerCase().includes(q)||r.status.toLowerCase().includes(q)||r.class.toLowerCase().includes(q);
  }).sort((a,b)=>{
          ...
          <div style={{ padding:'12px 16px', display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:10, borderBottom:'1px solid #F3F4F6' }}>
            ...
            <div style={{ display:'flex', alignItems:'center', gap:8 }}>
              ...
              <input value={search} onChange={e=>setSearch(e.target.value)}
                style={{ padding:'5px 10px', border:'1px solid #D1D5DB', borderRadius:6, fontSize:13, outline:'none', width:200 }} placeholder="Name, ID, status…"/>
            </div>
          </div>
            ...
            <table style={{ width:'100%', borderCollapse:'collapse', fontSize:13 }}>
              ...
              <tbody>
                {filtered.length===0 ? (
                  ...
                  const sc = STATUS_COLORS[r.status.toLowerCase()] || { color:'#374151', bg:'#F3F4F6' };
                  return (
