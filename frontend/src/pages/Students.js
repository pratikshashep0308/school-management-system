  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // ── Transport assignment (route, bus, pickup/drop stops) ──
        try {
          const tRes = await api.get('/transport/assignments', { params: { student: s._id } });
          const list = tRes.data?.data || [];
          setTransport(list[0] || null);   // a student has at most one active assignment
        } catch {
          setTransport(null);
        }
      } catch {}
